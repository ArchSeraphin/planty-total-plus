...
if (!class_exists('Total_Plus_Plugin_Installer')) {

    final class Total_Plus_Plugin_Installer {
        ...
        public function enqueue_scripts() {
            wp_enqueue_script('plugin-installer', get_template_directory_uri() . '/inc/total-plus/recommended-plugins/js/plugin-installer.js', array('jquery'), TOTAL_PLUS_VERSION, false);
            ...
            wp_localize_script('plugin-installer', 'PluginInstallerObject', array(
                ...
                'activate_btn' => esc_html__('Activate', 'total-plus'),
                'installed_btn' => esc_html__('Installed', 'total-plus'),
                'activating_btn' => esc_html__('Activating', 'total-plus'),
                'installing_btn' => esc_html__('Installing', 'total-plus'),
                'error_message' => esc_html__('Something went wrong. Plugin can not be installed.', 'total-plus'),
                'wait_message' => esc_html__('Please wait for the previous action to complete.', 'total-plus')
            ));
        }
        ...
        public static function generate_plugin_label($plugin) {
            ...
            switch ($status) {
                case 'install' :
                    $btn_label = esc_html__('Install', 'total-plus');
                    break;

                case 'inactive' :
                    $btn_label = esc_html__('Activate', 'total-plus');
                    break;

                case 'active' :
                    $btn_label = esc_html__('Installed', 'total-plus');
                    break;
            }
            return $btn_label;
        }
        ...
        public function plugin_installer_callback() {
            ...
            if (!current_user_can('install_plugins')) {
                wp_send_json(
                        array(
                            'success' => false,
                            'message' => esc_html__('Sorry, you are not allowed to install plugins on this site.', 'total-plus')
                        )
                );
                die();
            }
            ...
            if (!wp_verify_nonce($nonce, 'plugin_installer_nonce')) {
                wp_send_json(
                        array(
                            'success' => false,
                            'message' => esc_html__('Error - unable to verify nonce, please try again.', 'total-plus')
                        )
                );
                die();
            }
        ...
        public function plugin_offline_installer_callback() {
            ...
            if ($source) {
                ...
                wp_send_json(
                        array(
                            'success' => false,
                            'message' => __('Missing File Location.', 'total-plus')
                        )
                );
            }
        ...
        public function plugin_activation_callback() {
            ...
            if (file_exists($plugin_file)) {
                ...
                wp_send_json(
                        array(
                            'success' => false,
                            'message' => __('Error!! Plugin not activated.', 'total-plus')
                        )
                );
            }
        ...
        public function plugin_deactivation_callback() {
            ...
            if (file_exists($plugin_file)) {
                ...
                wp_send_json(
                        array(
                            'success' => false,
                            'message' => __('Error!! Plugin not deactivated.', 'total-plus')
                        )
                );
            }
        ...
        public function get_local_dir_path($file_location, $slug) {
            ...
            if (isset($file_location)) {
                ...
                $url = wp_nonce_url(admin_url('themes.php?page=total-plus-install-plugins'), 'remote-file-installation');
...
Total_Plus_Plugin_Installer::get_instance();
