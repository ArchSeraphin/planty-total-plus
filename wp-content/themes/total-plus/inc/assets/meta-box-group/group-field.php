...
class RWMB_Group_Field extends RWMB_Field {
	...
	public static function admin_enqueue_scripts() {
		...
		wp_localize_script(
			...
			'RWMB_Group',
			array(
				'confirmRemove' => __( 'Are you sure you want to remove this group?', 'total-plus' ),
			)
		);
	}
	...
	protected static function output_collapsible_elements( $field, $meta ) {
		...
		if ( $field['clone'] ) {
			echo '<a href="javascript:;" class="rwmb-group-remove">', esc_html__( 'Remove', 'total-plus' ), '</a>';
		}
		echo '</div>';
	...
	public static function normalize( $field ) {
		...
		$field = wp_parse_args(
			...
			array(
				'collapsible'   => false,
				'save_state'    => false,
				'group_title'   => $field['clone'] ? __( 'Entry {#}', 'total-plus' ) : __( 'Entry', 'total-plus' ),
