...
class RWMB_File_Field extends RWMB_Field {
	...
	public static function admin_enqueue_scripts() {
		...
		RWMB_Helpers_Field::localize_script_once(
			...
			array(
				...
				'maxFileUploadsSingle' => __( 'You may only upload maximum %d file', 'total-plus' ),
				// Translators: %d is the number of files in plural form.
				'maxFileUploadsPlural' => __( 'You may only upload maximum %d files', 'total-plus' ),
			)
		);
	}
	...
	public static function ajax_delete_file() {
		...
		if ( ! in_array( $attachment, $field_value ) ) {
			wp_send_json_error( __( 'Error: Invalid file', 'total-plus' ) );
		}
		...
		wp_send_json_error( __( 'Error: Cannot delete file', 'total-plus' ) );
	}
	...
	public static function html( $meta, $field ) {
		...
		$i18n_more = apply_filters( 'rwmb_file_add_string', esc_html_x( '+ Add new file', 'file upload', 'total-plus' ), $field );
	...
	protected static function file_html( $file, $index, $field ) {
		$i18n_delete = apply_filters( 'rwmb_file_delete_string', esc_html_x( 'Delete', 'file upload', 'total-plus' ) );
		$i18n_edit   = apply_filters( 'rwmb_file_edit_string', esc_html_x( 'Edit', 'file upload', 'total-plus' ) );
