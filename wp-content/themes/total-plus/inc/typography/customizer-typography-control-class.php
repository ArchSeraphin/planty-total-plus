 ...
 * @access public
 */
class Total_Plus_Typography_Control extends WP_Customize_Control {
    ...
    public function __construct($manager, $id, $args = array()) {
        ...
        $this->l10n = wp_parse_args(
                $this->l10n, array(
            'family' => esc_html__('Font Family', 'total-plus'),
            'style' => esc_html__('Font Weight/Style', 'total-plus'),
            'text_transform' => esc_html__('Text Transform', 'total-plus'),
            'text_decoration' => esc_html__('Text Decoration', 'total-plus'),
            'size' => esc_html__('Font Size', 'total-plus'),
            'line_height' => esc_html__('Line Height', 'total-plus'),
            'letter_spacing' => esc_html__('Letter Spacing', 'total-plus'),
            'typocolor' => esc_html__('Font Color', 'total-plus')
                )
        );
    }
        ...
        <ul>
            ...
            <li class="typography-color">
                ...
                <div class="customize-control-content">
                    <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e('Hex Value', 'total-plus'); ?>" {{{ data.typocolor.link }}} value="{{ data.typocolor.value }}"  />
                </div>
            </li>
            <# } #>
    ...
    function get_google_font_families() {
        ...
        $total_plus_google_font = total_plus_google_font_array();

        foreach ($total_plus_google_font as $key => $value) {
    ...
    public function get_font_weight_choices() {
        if ($this->settings['family']->id) {
            ...
            $total_plus_font = array_merge($total_plus_standard_font, $total_plus_google_font);
            ...
            $font_array = total_plus_search_key($total_plus_font, 'family', $get_font_family);

            $variants_array = $font_array['0']['variants'];
            return $variants_array;
        } else {
            return array(
                '400' => esc_html__('Normal', 'total-plus'),
                '700' => esc_html__('Bold', 'total-plus')
            );
        }
    }
    ...
    public function get_text_decoration_choices() {
        return array(
            ...
            'underline' => esc_html__('Underline', 'total-plus'),
            'line-through' => esc_html__('Line-through', 'total-plus'),
            'overline' => esc_html__('Overline', 'total-plus')
        );
    }
    ...
    public function get_text_transform_choices() {
        return array(
            'none' => esc_html__('None', 'total-plus'),
            'uppercase' => esc_html__('Uppercase', 'total-plus'),
            'lowercase' => esc_html__('Lowercase', 'total-plus'),
            'capitalize' => esc_html__('Capitalize', 'total-plus')
        );
    }
